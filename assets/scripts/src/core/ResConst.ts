 ...
 * @LastEditTime: 2023-03-11 17:56:50
 * @FilePath: \SanGuo2.4\assets\script\app\core\res\ResConst.ts
 * @Description:
 */

import { Asset, JsonAsset, Node, Skeleton, SpriteFrame, Texture2D, Vec2 } from "cc"
...
export interface IAssetEx extends Asset {
    _uuid: string,
    decRef: (autoRelease?: boolean) => Asset
}
// export interface ITexture2DEx extends IAssetEx{

// }
// export interface ISpriteFrameEx extends SpriteFrame {
//     _uuid: string
//     _name: string
//     // _original: {
//     //     _texture?: Texture2D,
...
export interface IPlistAsset extends JsonAsset {
...
export interface IShowPrefabResultData {
    parent?: Node,
    func?: (err: Error, node: Node, customData?: any) => void,
    isOnce?: boolean
    target?: any,
    ...
    public constructor(pathNoSuffix: string | string[], suffix: string = ESuffix.Png) {
        if (pathNoSuffix) {
            if (typeof pathNoSuffix === 'string') {
                this._name = UtilsString.GetFileName(pathNoSuffix);
                this._pathNoSuffix = [pathNoSuffix];
                if (pathNoSuffix.indexOf('action') >= 0) {
                    this._mergeJsonPathNoSuffix = [UtilsString.ReplaceFileNameForAnim(pathNoSuffix, '_cfg')];
                }
            } else {
                this._name = UtilsString.GetFileName(pathNoSuffix[0]);
                this._pathNoSuffix = pathNoSuffix;
                this._mergeJsonPathNoSuffix = [];
                for (let i = 0; i < pathNoSuffix.length; i++) {
                    if (pathNoSuffix[i].indexOf('action') >= 0) {
                        this._mergeJsonPathNoSuffix.push(UtilsString.ReplaceFileNameForAnim(pathNoSuffix[i], '_cfg'));
                    }
                }
            }
            this._suffix = suffix;
...
export interface IResBase {
    ...
    loadTexture: (path: string, loadInfo: LoadInfo, callback: CompleteCallback<Texture2D>, target?: object, ...args: any[]) => void
    loadPlist: <T>(path: string, loadInfo: LoadInfo, callback: CompleteCallback<T>, target?: object, plistName?: string, ...args: any[]) => void
    loadSpine: (loadInfo: LoadInfo, callback: CompleteCallback<Skeleton>, target: object, ...args: any[]) => void
    decRef: (asset: Asset, force?: boolean) => void
}
