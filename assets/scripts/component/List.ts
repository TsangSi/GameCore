 ...
 ***************************************** */
import {
 Node, Component, Enum, tween, _decorator, EventHandler, Tween, ScrollView, Prefab, Layout, Vec2, Size, NodePool, isValid, instantiate, Vec3, Widget, UITransform, CCFloat, CCBoolean, CCInteger,
} from 'cc';
...
export default class List extends Component {
    ...
    set numItems(val: number) {
        ...
        if (t._virtual) {
            ...
            if (t.frameByFrameRenderNum > 0) {
                // 先渲染几个出来
                const len: number = t.frameByFrameRenderNum > t._numItems ? t._numItems : t.frameByFrameRenderNum;
                for (let n = 0; n < len; n++) {
    ...
    onDestroy() {
        ...
        while (t._pool && t._pool.size()) {
            const node = t._pool.get();
            node.destroy();
        }
    }
    ...
    _init() {
        const t: any = this;
        if (t._inited || (Utils.isNullOrUndefined(t.tmpPrefab) && Utils.isNullOrUndefined(t.tmpNode))) {
            return;
        }
        ...
        const n: Node = instantiate(t.templateType == TemplateType.PREFAB ? t.tmpPrefab : t.tmpNode);
        n.active = true;
        t.setTemplateItem(n);
        ...
        switch (t._align) {
            ...
            case Layout.Type.GRID:
                switch (t._startAxis) {
                    case Layout.AxisDirection.HORIZONTAL:
                        // 计算列数
                        const trimW: number = t._contentUt.width - t._leftGap - t._rightGap;
                        t._colLineNum = Math.floor((trimW + t._columnGap) / (t._itemSize.width + t._columnGap));
                        t._sizeType = true;
                        break;
                    case Layout.AxisDirection.VERTICAL:
                        // 计算行数
                        const trimH: number = t._contentUt.height - t._topGap - t._bottomGap;
    ...
    _onScrolling(ev: Event = null) {
        ...
        let vTop: number; let vRight: number; let vBottom: number; let vLeft: number;
        if (this._sizeType) {
            vTop = this.viewTop;
    ...
    getItemPos(id: number) {
        if (this._virtual) { return this._calcItemPos(id); } else if (this.frameByFrameRenderNum) return this._calcItemPos(id);
    ...
    adhere() {
        ...
        t.adhering = true;
        t._calcNearestItem();
        const offset: number = (t._sizeType ? t._topGap : t._leftGap) / (t._sizeType ? t._thisNodeUt.height : t._thisNodeUt.width);
    ...
    aniDelItem(listId: number, callFunc: Function, aniType: number) {
        ...
        listItem.showAni(aniType, () => {
            ...
            // 后面的Item向前怼的动效
            const sec = 0.2333;
            let twe: Tween<Node>; let haveCB: boolean;
    ...
    scrollTo(listId: number, timeInSecond = 0.5, offset: number = null, overStress = false) {
        ...
        const runScroll = Math.abs((t._scrollPos != null ? t._scrollPos : viewPos) - comparePos) > 0.5;
