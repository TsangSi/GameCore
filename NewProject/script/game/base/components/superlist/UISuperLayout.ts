...
export default class UISuperLayout extends cc.Component {
    ...
    public getContentSize() {
        const size = this.getReallySize();
        const viewSize = this.scrollView.view.getContentSize();
    ...
    public async total(value: number) {
        ...
        const dataOffset = this.getDataOffset(value); // 获取数据偏移量（根据value相对于 _maxItemTotal 计算增加、减少的数量）
        const reallyOffset = this.getReallyOffset(dataOffset); // 获取真实的数据偏移（Grid模式 功能用于判断是否需要偏移header来将下方填满）
    ...
    public getBrotherNode(node: cc.Node) {
        const index = this.getSiblingIndex(node) - 1; // 此 getSiblingIndex 非 this.node.getSiblingIndex
        return this._children[index];
    }
    /** 是否是一组item中第一个（垂直滑动中 一组item 就是单行的所有列 、水平滑动中 一组item 就是单列中所有行） */
    public isGroupHeader(node: cc.Node): boolean {
        const xOry = this.getGroupHeader(node);
        const pos = this.vertical ? cc.v2(xOry.x, 0) : cc.v2(0, xOry.y);
        const self = this.vertical ? cc.v2(node.x, 0) : cc.v2(0, node.y);
    ...
    public isGroupFooter(node: cc.Node): boolean {
        const xOry = this.getGroupFooter(node);
        const pos = this.vertical ? cc.v2(xOry.x, 0) : cc.v2(0, xOry.y);
        const self = this.vertical ? cc.v2(node.x, 0) : cc.v2(0, node.y);
        return self.fuzzyEquals(pos, EPSILON);
    }
    ...
    public getGroupRightX(node: cc.Node, relative: cc.Node) {
        ...
        const prevWidth = relative.x + this.getScaleWidth(relative) * (1 - relative.anchorX);
        const selfWidth = this.getScaleWidth(node) * node.anchorX;
    ...
    public getGroupLeftX(node: cc.Node, relative: cc.Node) {
        ...
        const prevWidth = relative.x - this.getScaleWidth(relative) * relative.anchorX;
        const selfWidth = this.getScaleWidth(node) * (1 - node.anchorX);
        return prevWidth - selfWidth - this.spacing.x;
    }
    ...
    public getGroupBottomY(node: cc.Node, relative: cc.Node) {
        const prevHeight = relative.y - this.getScaleHeight(relative) * relative.anchorY;
        const selfHeight = this.getScaleHeight(node) * (1 - node.anchorY);
    ...
    public getGroupTopY(node: cc.Node, relative: cc.Node) {
        const prevHeight = relative.y + this.getScaleHeight(relative) * (1 - relative.anchorY);
        const selfHeight = this.getScaleHeight(node) * node.anchorY;
        return prevHeight + selfHeight + this.spacing.y;
    }
    ...
    public isOutOfBoundaryHeader(node: cc.Node, multiple: number = 1.5) {
        const width = node.width * this.getUsedScaleValue(node.scaleX) * multiple;
        const height = -node.height * this.getUsedScaleValue(node.scaleY) * multiple;
        const offset = this.scrollView.getHowMuchOutOfBoundary(cc.v2(width, height));
        return offset;
    }
    ...
    private onChangePosition() {
        const flag = this.isScrollToFooter; // this.isScrollToFooter = true 向下滑动 false 向上滑动
        if (this.headerToFooter) {
        ...
        if (this.vertical && this.node.y == 0 || this.horizontal && this.node.x == 0) {
            if (this.header) this.header.setPosition(this.getGroupHeader(this.header));
        }
    }
    public resetItemSize() {
        ...
        // 改变头部位置
        if (this.header) {
            const pos = this.getGroupHeader(this.header);
            if (this.vertical) {
                this.header.x = pos.x;
            } else {
                this.header.y = pos.y;
    ...
    private headerToFooterWatchChilds(flag) {
        const children = this.getTempChildren();
    ...
    private getDataOffset(value: number) {
        ...
        if (this.footer && this.footer.index + 1 >= value) {
            const offset = this.footer.index + 1 - value;
    ...
    */
    private getReallyOffset(dataOffset: number) {
        ...
        if (dataOffset > 0) { // 代表增加item 表格模式下 通过偏移头部来让下方填满 填满后停止偏移
            for (let i = 0; i < dataOffset; i++) {
                ...
                const pos = this.header.getPosition();
    ...
    private removeChilds(value: number) {
        // 有多余的item 需要删除
        const length = this.node.childrenCount - value;
        // 删除掉多余的item
        for (let i = 0; i < length; i++) {
            const child = this.footer;
            this.remChild(child);
            child.destroy();
    ...
    private async asyncCreateItem(value: number) {
        ...
        // 开始分帧创建item
        const total = value - this.node.childrenCount; // 计算当前应该创建的总数
        this._gener = this.getGeneratorLength(total, () => {
            const child = this.createItem();
            child.index = this.node.childrenCount;
            this.addChild(child);
            // 获取或添加 UISuperItem
            const spuerItem = child.getComponent(UISpuerItem) || child.addComponent(UISpuerItem);
            ...
            if (this.node.childrenCount == 1) {
                const pos = this.getGroupHeader(this.header); // 获取一组item中头部位置
                this.header.setPosition(pos);
                /**
            ...
            const execute = () => {
                ...
                for (let iter = gen.next(); ; iter = gen.next()) {
                    ...
                    if (new Date().getTime() - startTime > duration) {
                        this.scheduleOnce(() => execute(), cc.director.getDeltaTime() * 1000);
                        return;
                    }
                }
            };
