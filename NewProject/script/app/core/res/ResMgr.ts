...
import { RemoveSpine } from '../../../game/base/spine/SpineCmp';
...
import { UtilBool } from '../../base/utils/UtilBool';
import { UtilString } from '../../base/utils/UtilString';
import { CCResMgr } from '../../engine/CCResMgr';
import BundleMgr, { BundleType } from './BundleMgr';
import { RemoveSpriteAtlas } from './CreateAnimaForSheet';
import {
    AssetType,
    BigAssetType,
    CompleteCallback,
    ...
    IAssetPrototype,
    IResBase,
    IResLoadOps, IShowPrefabResultData, ISpriteAtlasEx, ISpriteFrameEx, ITexture2DEx, LoadInfo,
} from './ResConst';
...
@ccclass('ResMgr')
export class ResMgr implements IResBase {
    private static _I: ResMgr = null;
    public static get I(): ResMgr {
        if (this._I == null) {
            this._I = new ResMgr();
    ...
    /** 等待删除列表 */
    private waitDelete: { [uuid: string]: { asset: cc.Asset, time: number } } = cc.js.createMap(true);
    /** 清除下载任务记录 */
